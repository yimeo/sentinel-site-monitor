...
if [[ "${NODE_VERSION%%.*}" -lt 18 ]]; then
  runuser -u "$APP_USER" -- env HOME=/var/lib/site-monitor/home PATH="$NODE_DIR/bin:$PATH" "$NODE_DIR/bin/npm" install --omit=dev --legacy-peer-deps --ignore-scripts --no-audit --no-fund
else
